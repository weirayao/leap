...
class CondAfflineVAESynthetic(pl.LightningModule):
    ...
    def __init__(
        ...
        # Separate encoder for past frames
    ...
    def training_step(self, batch, batch_idx):
        ...
        xt_ = xt_.squeeze()
        zt = self.past_enc(xt)
        x_recon, mut_, logvart_, zt_ = self.net(xt_)

        # VAE ELBO loss: recon_loss + kld_loss
        recon_loss = reconstruction_loss(xt_, x_recon, self.decoder_dist)
        zt = zt.view(batch_size, -1, self.z_dim)

        # KLD divergence DKL[q(z_t|x_t) || p(z_t|{z_t-tau})]
        ...
        kld_laplace = torch.sum(log_qz_laplace, dim=-1) - log_pz_laplace
        kld_laplace = kld_laplace.mean()       
        
        loss = recon_loss + self.gamma * kld_laplace  
    ...
    def validation_step(self, batch, batch_idx):
        ...
        zt_recon = mut_.T.detach().cpu().numpy()
