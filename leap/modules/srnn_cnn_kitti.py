...
from .components.conv import KiTTiConvEncoder, KiTTiConvDecoder
...
class SRNNConv(pl.LightningModule):
    ...
    def __init__(
        ...
        # Recurrent inference
        if infer_mode == 'R':
            self.enc = KiTTiConvEncoder(z_dim=z_dim, nc=nc, hidden_dim=hidden_dim)
            self.dec = KiTTiConvDecoder(z_dim=z_dim, nc=nc, hidden_dim=hidden_dim)
