...
def gen_Ball(info):
    ...
    for i in tqdm(range(n_rollout)):
        ...
        for j in range(time_step):
            ...
            act = np.random.laplace(loc=0.0, scale=1.0, size=(n_obj, 2)) * 100
...
class PhysicsDatasetTwoSample(Dataset):
    ...
    def retrieve_by_index(self, idx):
        ...
        if self.load_img:
            ...
            return None, kps_gt, graph_gt, actions, src_phase
...
class PhysicsDatasetPCL(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        idx_rnd = random.randint(0, self.__len__() -1)
        imgsr, kps_gtr, graph_gtr, actionsr, src_phaser = self.retrieve_by_index(idx_rnd)
        xt_cur, xt_his = self.seq_to_pairs(imgs)
        xtr_cur, xtr_his = self.seq_to_pairs(imgsr)
...
class PhysicsDatasetTuple(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = {"xt": imgs[:-1],
                  "xt_": imgs[-1:],
                  "yt": kps_gt[:-1,:,:2].reshape(-1,10),
                  "yt_": kps_gt[-1:,:,:2].reshape(-1,10)
                 }        
