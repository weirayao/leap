...
class AfflineVAESynthetic(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        x = torch.cat((xt, xt_), dim=1)
        x = x.view(-1, self.input_dim)
        x_recon, mu, logvar, z = self.net(x,xt)
        
        # Normal VAE loss: recon_loss + kld_loss
        recon_loss = reconstruction_loss(xt_.squeeze(), x_recon, self.decoder_dist)
        ...
        # loss = self.lam * recon_loss + self.beta * kld_normal + self.gamma * kld_laplace 
        loss = self.lam * recon_loss + self.gamma * kld_laplace
